...
st.markdown(
    '<p style="font-family: sans-serif; color: White; font-size: 50px;">Company Policy Q&A Bot with RAG</p>',
...
if go_button and input_text.strip():
    with st.spinner("I'm getting answers to your Queries!"):
        response = brain.company_rag_response(index=st.session_state['vector_index'], question=input_text)
        st.write(response)
